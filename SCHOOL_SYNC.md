...
Unity 프로젝트: `Team-Speakkki`. 공통 DLL 위치: `Team-Speakkki/Assets/Plugins/SchoolSync`.
...
위치는 직접 계산한다. 상대 ID별 생성·갱신·제거와 맵 생성·충돌은 학생이 구현한다. OnDestroy에서 `server?.Dispose()`를 호출한다. API 상세는 `Team-Speakkki/Assets/Plugins/SchoolSync/API.md`에 있다. 내장 맵은 원작 World 1-1 이미지에서 추출한 지상·지하 그리드다.
